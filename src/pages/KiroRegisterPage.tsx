...
export function KiroRegisterPage() {
  const store = useRegistrationStore();
  const progressPercent =
    store.progress.total > 0 ? (store.progress.current / store.progress.total) * 100 : 0;

  const [imapEmail, setImapEmail] = useState('');
  const [imapPassword, setImapPassword] = useState('');
  const [imapTesting, setImapTesting] = useState(false);
  const [mode, setMode] = useState<'imap' | 'bindcard'>('imap');
  ...
  return (
    <div className="animate-in">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title" style={{ background: 'linear-gradient(135deg, #f97316, #ef4444)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            {mode === 'bindcard' ? 'Kiro 自动绑卡' : 'Kiro 批量注册'}
          </h1>
          <p className="page-subtitle">
            {mode === 'bindcard' ? '批量登录已注册账号并自动绑定支付卡' : '通过 AWS Builder ID 自动化注册 Kiro 账号'}
          </p>
        </div>
      ...
      <div className="card" style={{ marginBottom: 16, padding: '12px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)' }}>🎯 注册模式</span>
          <div style={{ display: 'flex', gap: 6, flex: 1 }}>
            <button className={`btn btn-sm ${mode === 'imap' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setMode('imap')} style={{ flex: 1 }} disabled={store.isRunning}>
              📧 IMAP 模式
            </button>
            <button className={`btn btn-sm ${mode === 'bindcard' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setMode('bindcard')} style={{ flex: 1 }} disabled={store.isRunning}>
              💳 自动绑卡
            </button>
          </div>
        </div>
        ...
        <div className="column-left">
            ...
            {sections.imap && (
              <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ border: '1px dashed rgba(255,255,255,0.15)', borderRadius: 10, padding: 12 }}>
                  ...
                  <button className="btn btn-primary btn-full btn-sm" onClick={handleAddImap} disabled={store.isRunning || imapTesting || !imapEmail || !imapPassword}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {store.isRunning ? (
              ...
              <button className="btn btn-full" style={{ background: 'linear-gradient(135deg, #f97316, #ef4444)', color: 'white', boxShadow: '0 4px 16px rgba(249,115,22,0.3)' }} onClick={store.startRegistration}>
                ▶ {mode === 'bindcard' ? '开始绑卡' : '开始注册'}
              </button>
            )}
          </div>

          <div className="alert info">
            <strong>💡 Kiro 注册说明</strong>
            <ul style={{ paddingLeft: 16, marginTop: 4, lineHeight: 1.8 }}>
              <li>Kiro 使用 AWS Builder ID，注册后需授权访问</li>
              <li>验证码来自 AWS (no-reply@signin.aws)</li>
              <li>需确保 IMAP 可接收 AWS 邮件</li>
            </ul>
          </div>
        </div>
